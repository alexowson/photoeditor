...
class MultiTouchListener implements OnTouchListener {
    ...
    MultiTouchListener(View deleteView, PhotoEditorView parentView,
                       ImageView photoEditImageView, OnPhotoEditorListener onPhotoEditorListener) {
    ...
    private void firePhotoEditorSDKListener(View view, boolean isStart) {
        if (view instanceof TextView) {
            if (onMultiTouchListener != null) {
                if (onPhotoEditorListener != null) {
                    if (isStart)
                        onPhotoEditorListener.onStartViewChangeListener(ViewType.TEXT);
                    else
                        onPhotoEditorListener.onStopViewChangeListener(ViewType.TEXT);
                }
            } else {
                if (onPhotoEditorListener != null) {
                    if (isStart)
                        onPhotoEditorListener.onStartViewChangeListener(ViewType.EMOJI);
                    else
                        onPhotoEditorListener.onStopViewChangeListener(ViewType.EMOJI);
                }
            }
        } else {
            if (onPhotoEditorListener != null) {
